#! /bin/python3

import csv, math, numpy

class RandomNumberGenerator:
    def __init__(self, seedVaule=None):
        self.__seed=seedVaule
    def nextInt(self, low, high):
        m = 2147483647
        a = 16807
        b = 127773
        c = 2836
        k = int(self.__seed / b)
        self.__seed = a * (self.__seed % b) - k * c;
        if self.__seed < 0:
            self.__seed = self.__seed + m
        value_0_1 = self.__seed
        value_0_1 =  value_0_1/m
        return low + int(math.floor(value_0_1 * (high - low + 1)))
    def nextFloat(self, low, high):
        low*=100000
        high*=100000
        val = self.nextInt(low,high)/100000.0
        return val



gen = RandomNumberGenerator(666)

n = 10
a_min = 5.0
a_max = 35.0
b_min = 5.0
b_max = 35.0
r_min = 1.0
r_max = 4.0

a = []
b = [] 
r = []

for i in range(n):
    a.append(gen.nextFloat(a_min, a_max) )
    b.append(gen.nextFloat(b_min, b_max) )
    r.append(gen.nextFloat(r_min, r_max) )

x_0 = gen.nextFloat(a_min, a_max)
y_0 = gen.nextFloat(a_min, a_max)

with open('data.csv', mode='w') as data_out:
    out = csv.writer(data_out, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
    out.writerow([x_0, y_0])
    out.writerows([a, b, r])
